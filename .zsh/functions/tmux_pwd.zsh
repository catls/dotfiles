# Instructions
# ------------
#
# 1. source this file and Add a function in your chpwd()
#    source tmux-pwd.sh
#
#    chpwd() {
#        tmux_chpwd
#    }
#
# 2. Add a shortcut in your ~/.tmux.conf file:
#
#    set -g status-right '#[fg=cyan,bg=black] "#(cat /tmp/tmux/tmux-pwd_`tmux display -p "#S-#I-#P"`)" :#(tmux display -p "#I-#P") '
#
TMUX_TMP_DIR="/tmp/tmux"
tmux_chpwd() {
	#[ -n "$TMUX" ] && tmux setenv TMUXPWD_$(tmux display -p "#I") $PWD
	[ ! -d $TMUX_TMP_DIR ] && mkdir -p $TMUX_TMP_DIR
	pwd > ${TMUX_TMP_DIR}/tmux-pwd_`tmux display -p "#S-#I-#P"`
}

set_tmux_status(){
	local string="$1"
	echo $string > ${TMUX_TMP_DIR}/tmux-pwd_`tmux display -p "#S-#I-#P"`
}
# # Get the current path in the segment.
# get_tmux_cwd(){
# 	local env_name=$(tmux display -p "TMUXPWD_#D" | tr -d %)
# 	local env_val=$(tmux show-environment | grep --color=never "$env_name")
# 	# The version below is still quite new for tmux. Uncomment this in the future :-)
# 	#local env_val=$(tmux show-environment "$env_name" 2>&1)
# 
# 	if [[ ! $env_val =~ "unknown variable" ]]; then
# 		local tmux_pwd=$(echo "$env_val" | sed 's/^.*=//')
# 		echo "$tmux_pwd"
# 	fi
# }

